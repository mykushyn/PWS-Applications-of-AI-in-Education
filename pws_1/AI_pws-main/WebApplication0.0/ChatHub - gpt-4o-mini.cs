...
//========================================= EIND DISCONNECT HANDLING ========================================
    ...
    private async Task<string> GetAIResponse(string userMessage, string context, string bookName, List<ChatMessage> conversationHistory)
    {
        ...
        if (!string.IsNullOrEmpty(context))
        {
            ...
            { 
                ...
                content = $@"Here is relevant information from the textbook:
                    {context}
                    You MUST use ONLY information from these textbooks to teach. Base your guidance on these concepts." 
            });
        }
        ...
        messages.Add(new
        {
            role = "user",
            content = $@"{userMessage}
                Remember: Guide me through this. Don't give me the final answer directly." 
        });
...
Step 2: Explaine the concept needed
Step 3: Ask a guiding question
Step 4: Provide a hint if needed
Step 5: Wait for them to respond before continuing
